angular.module('app').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/src/components/dialogs/dataschemaImport/githubHook/githubHookFileStep.html',
    "<md-list><md-icon class=\"material-icons\" ng-if=\"dialog.currentStep().hasParentFolder()\" ng-click=\"dialog.currentStep().goToParentFolder()\">arrow_upward</md-icon><md-list-item ng-repeat=\"file in dialog.currentStep().getFiles() track by $index\" ng-click=\"dialog.currentStep().selectFile(file)\" ng-class=\"{selectedListItem: dialog.currentStep().selectedFile === file}\">{{file.getName()}}</md-list-item></md-list>"
  );
  ...
  $templateCache.put('app/src/components/dialogs/dataschemaImport/githubHook/githubHookRepoStep.html',
    "<md-content layout=\"rows\"><md-list flex=\"80\"><md-list-item ng-repeat=\"repo in dialog.currentStep().getRepos() track by $index\" ng-click=\"dialog.currentStep().selectRepo(repo)\" ng-class=\"{selectedListItem: dialog.currentStep().selectedRepo === repo}\">{{repo.name}}</md-list-item></md-list><md-list ng-if=\"dialog.currentStep().someRepoIsSelected\" flex=\"20\"><md-list-item ng-repeat=\"branch in dialog.currentStep().getBranches() track by $index\" ng-click=\"dialog.currentStep().selectBranch(branch)\" ng-class=\"{selectedListItem: dialog.currentStep().selectedBranch === branch}\">{{branch.name}}</md-list-item></md-list></md-content>"
  );
  ...
  $templateCache.put('app/src/components/toolbox/toolbox.html',
    "<md-content style=\"height:100%\" layout=\"column\"><md-subheader class=\"md-primary bottom-border\">CONTROLS</md-subheader><md-content style=\"padding-top: 10px\"><div layout=\"row\" style=\"padding-right: 17px; padding-left: 17px\"><md-switch class=\"md-primary\" style=\"margin-top: 0px; margin-bottom: 0px\" md-no-ink ng-model=\"toolbox.configService.enableFilter\">Hide used elements</md-switch><span flex></span><md-icon class=\"material-icons\" ng-if=\"!toolbox.isParentFolder()\" ng-click=\"toolbox.clickedBack()\">arrow_upward</md-icon></div></md-content><md-content flex class=\"md-padding tree-view\" style=\"margin-bottom:150px; padding-top: 0px\"><div ui-tree=\"toolbox.treeOptions\" id=\"toolbox-root\" data-drag-delay=\"100\" data-clone-enabled=\"true\" data-nodrop-enabled=\"true\"><ol ui-tree-nodes ng-model=\"toolbox.toolboxService.elements\"><li ng-repeat=\"element in toolbox.toolboxService.elements\" ui-tree-node ng-hide=\"toolbox.shouldHide(element)\"><div ui-tree-handle class=\"tree-node tree-node-content\" ng-class=\"{'toolbox-element-placed': toolbox.isPlaced(element)}\" ng-style=\"{cursor: (element.isObject() ? 'default' : 'move')}\"><md-icon ng-if=\"element.isObject()\" ng-click=\"toolbox.clickedIcon(element)\" class=\"material-icons\">{{element.getIcon()}}</md-icon><span ng-if=\"element.isObject()\" ng-click=\"toolbox.clickedIcon(element)\" style=\"overflow:hidden\" class=\"left-margin\">{{element.getLabel()}}</span><md-icon ng-if=\"!element.isObject()\" class=\"material-icons\">{{element.getIcon()}}</md-icon><span ng-if=\"!element.isObject()\" style=\"overflow:hidden\" class=\"left-margin\">{{element.getLabel()}}</span> <a class=\"pull-right left-margin\" ng-hide=\"!toolbox.canBeRemoved(element)\" ng-click=\"toolbox.removeDataElement(element)\"><md-icon class=\"btn material-icons\">delete</md-icon></a></div></li></ol></div></md-content><md-content class=\"toolbox-bottom-control top-border\" ng-controller=\"ToolboxBottomController as toolboxBottom\"><div ng-show=\"toolboxBottom.newElementConfig.hasEnum\" ng-include=\"'enumProperties.html'\"></div><div ng-show=\"toolboxBottom.showAdvanced\" ng-include=\"toolboxBottom.advancedTemplate\"></div><md-button ng-show=\"toolboxBottom.advancedTemplate!=null\" class=\"md-primary md-ink-ripple toolbox-advanced-button\" ng-click=\"toolboxBottom.showAdvanced = !toolboxBottom.showAdvanced\">Advanced</md-button><div layout=\"row\"><div flex class=\"toolbox-input-datatype-box\" ng-repeat=\"(typeLabel, typeTemplate) in toolboxBottom.elementTypes\" ng-click=\"toolboxBottom.changeType(typeLabel, typeTemplate)\" ng-class=\"{'toolbox-input-datatype-box-active': toolboxBottom.newElementTypeLabel===typeLabel}\">{{typeLabel}}</div></div><div layout=\"row\" layout-align=\"space-around center\"><md-input-container flex=\"70\" style=\"margin-top:20px; margin-bottom:0px\"><input id=\"inputLabel\" ng-enter=\"toolboxBottom.addNewElement()\" ng-model=\"toolboxBottom.newElementLabel\" class=\"md-primary\" type=\"text\" placeholder=\"Add new attribute...\"></md-input-container><div flex=\"30\"><md-button class=\"md-raised md-primary toolbox-button-create\" ng-click=\"toolboxBottom.addNewElement()\">Create</md-button></div></div><!-- Templates for the advanced menus --><script type=\"text/ng-template\" id=\"basicProperties.html\"><md-checkbox ng-model=\"$parent.toolboxBottom.newElementConfig.required\">Required</md-checkbox>\n" +
    "            <md-checkbox ng-model=\"$parent.toolboxBottom.newElementConfig.hasEnum\">Enum</md-checkbox></script><script type=\"text/ng-template\" id=\"enumProperties.html\"><div layout=\"row\" layout-align=\"space-around center\">\n" +
    "\n" +
    "                <md-input-container flex=\"70\" style=\"margin-top:20px; margin-bottom:0px;\">\n" +
    "                    <input ng-enter=\"$parent.toolboxBottom.addEnumElement()\" ng-model=\"$parent.toolboxBottom.currentEnumElementLabel\" class=\"md-primary\" type=\"text\" placeholder=\"Add new enum element...\"/>\n" +
    "                </md-input-container>\n" +
    "                <div flex=\"30\">\n" +
    "                    <md-button class=\"md-raised md-primary toolbox-button-create\" ng-click=\"$parent.toolboxBottom.addEnumElement()\">\n" +
    "                        Add\n" +
    "                    </md-button>\n" +
    "                </div>\n" +
    "            </div>\n" +
    "\n" +
    "            <md-select ng-model=\"$parent.toolboxBottom.newElementConfig.enum\" multiple placeholder=\"Current elements\">\n" +
